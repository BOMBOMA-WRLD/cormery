/**
 * @file mistral.provider.ts
 *
 * Provider concret Mistral de CORMERY.
 *
 * Position dans l'architecture :
 *
 *   AI orchestration → AIProviderManager → MistralProvider → API Mistral
 *
 ...
 * et ne persiste aucune donnée utilisateur.
 *
 * Capacités réellement implémentées ici :
 * - `text_generation` et `chat` via `generate()` ;
 * - `structured_output` via `responseFormat` (`json_object` / `json_schema`) ;
 * - `tool_calling` : les outils sont transmis et les `tool_calls` retournés normalisés,
 *   mais JAMAIS exécutés par ce provider.
 *
...
const PROVIDER_ID: AIProviderId = 'mistral';

/** Nom lisible du provider. */
const PROVIDER_NAME = 'Mistral AI';

/**
 * Endpoint officiel utilisé lorsque `MISTRAL_BASE_URL` n'est pas configurée.
 * Documenté ici car c'est la seule valeur réseau en dur du fichier ; toute
 * installation spécifique (proxy, gateway) passe par la configuration.
 */
const DEFAULT_BASE_URL = 'https://api.mistral.ai';

/** Chemin de génération conversationnelle de l'API Mistral. */
const CHAT_COMPLETIONS_PATH = '/v1/chat/completions';

/** Chemin de listage des modèles, utilisé comme sonde légère par `getStatus()`. */
...
/* -------------------------------------------------------------------------- */
/*  Types de la charge utile Mistral (sortants)                               */
/* -------------------------------------------------------------------------- */

type MistralRole = 'system' | 'user' | 'assistant' | 'tool';

interface MistralToolCallPayload {
  readonly id: string;
  readonly type: 'function';
  readonly function: { readonly name: string; readonly arguments: string };
}

interface MistralMessagePayload {
  readonly role: MistralRole;
  readonly content: string;
  readonly tool_calls?: readonly MistralToolCallPayload[];
...
type MistralToolChoicePayload =
  | 'auto'
  | 'none'
  | 'any'
  | { readonly type: 'function'; readonly function: { readonly name: string } };

type MistralResponseFormatPayload =
  | { readonly type: 'text' }
...
interface MistralChatRequestPayload {
  readonly model: string;
  readonly messages: readonly MistralMessagePayload[];
  readonly stream: false;
  readonly temperature?: number;
  readonly max_tokens?: number;
  readonly tools?: readonly MistralToolPayload[];
  readonly tool_choice?: MistralToolChoicePayload;
  readonly response_format?: MistralResponseFormatPayload;
}
 ...
 */
export type MistralFetch = (input: string, init: RequestInit) => Promise<Response>;
...
export interface MistralProviderOptions {
  /** Clé API ; par défaut `envConfig.MISTRAL_API_KEY`. Jamais exposée après construction. */
  readonly apiKey?: string;
  /** Base URL ; par défaut `envConfig.MISTRAL_BASE_URL`, sinon l'endpoint officiel. */
  readonly baseUrl?: string;
  /** Timeout appliqué quand la requête n'en précise pas. */
  readonly defaultTimeoutMs?: number;
  /** Transport HTTP ; par défaut le `fetch` global de Node.js. */
  readonly fetchImpl?: MistralFetch;
}
 ...
 * `POST /v1/chat/completions`, applique le timeout et l'`AbortSignal` fournis,
 * puis normalise la réponse en {@link AIGenerationResponse}. Toute erreur —
 ...
 *
 * Comportement de `getStatus()` : sonde légère `GET /v1/models` avec un timeout
...
export class MistralProvider extends BaseAIProvider {
  ...
  private readonly fetchImpl: MistralFetch;
  ...
  public constructor(options: MistralProviderOptions = {}) {
    super();
    this.apiKey = options.apiKey ?? envConfig.MISTRAL_API_KEY;
    this.baseUrl = (options.baseUrl ?? envConfig.MISTRAL_BASE_URL ?? DEFAULT_BASE_URL).replace(
      /\/+$/,
      '',
    );
   ...
   * registre de modèles Mistral faisant autorité, et déclarer des identifiants
  ...
  private requireApiKey(model?: AIModelId): string {
    if (this.apiKey === undefined) {
      throw new AIProviderError({
        ...
        providerId: this.id,
        message: 'Mistral API credential is not configured.',
        model,
        retryable: false,
      });
    }
  ...
  /**
   * Transforme la requête CORMERY en charge utile Mistral.
   *
  ...
  private buildChatPayload(
    ...
    model: string,
  ): MistralChatRequestPayload {
    const messages: MistralMessagePayload[] = [];
    if (request.systemInstruction !== undefined && request.systemInstruction !== '') {
      messages.push({ role: 'system', content: request.systemInstruction });
    }
    for (const message of request.messages) {
      messages.push(...this.toMistralMessages(message));
    }
  ...
  /**
   * Convertit un message CORMERY en un ou plusieurs messages Mistral.
   *
   * Un message porteur de résultats d'outils produit un message Mistral par
   * résultat, car l'API corrèle chaque résultat à un `tool_call_id` unique.
   *
   * @param message - Message normalisé.
   * @returns Messages Mistral équivalents.
   * @throws {AIProviderError} Si le message contient une image (`vision` non supporté).
   */
  private toMistralMessages(message: AIMessage): MistralMessagePayload[] {
    const texts: string[] = [];
    const toolCalls: MistralToolCallPayload[] = [];
    const toolResults: MistralMessagePayload[] = [];
    ...
    if (content !== '' || toolCalls.length > 0) {
      messages.push({
        role: this.toMistralRole(message.role),
        content,
   ...
   * @returns Rôle Mistral équivalent (correspondance exacte, un à un).
   */
  private toMistralRole(role: AIMessage['role']): MistralRole {
    return role;
  }
   ...
   * @returns Outils au format Mistral, ou `undefined` si aucun outil n'est fourni.
   */
  private mapTools(
    tools: readonly AIToolDefinition[] | undefined,
  ): readonly MistralToolPayload[] | undefined {
   ...
   * @returns Politique Mistral équivalente (`required` correspond à `any`).
   */
  private mapToolChoice(choice: AIToolChoice | undefined): MistralToolChoicePayload | undefined {
   ...
   * @returns Format Mistral équivalent, ou `undefined` pour le format textuel implicite.
   */
  private mapResponseFormat(
    format: AIResponseFormat | undefined,
  ): MistralResponseFormatPayload | undefined {
  ...
  private transportError(
    ...
    if (timedOut) {
      return new AIProviderError({
        code: 'timeout',
        providerId: this.id,
        message: `Mistral API request exceeded the ${String(timeoutMs)} ms timeout.`,
        model,
        retryable: true,
      });
    }
    if (callerAborted) {
      return new AIProviderError({
        code: 'provider_error',
        providerId: this.id,
        message: 'Mistral API request was aborted by the caller.',
        model,
        retryable: false,
      });
    }
    return new AIProviderError({
      ...
      providerId: this.id,
      message: 'Network failure while contacting the Mistral API.',
      model,
      retryable: true,
    });
  }
  ...
  private async readJson(response: Response, model?: AIModelId): Promise<unknown> {
    try {
      ...
      throw new AIProviderError({
        code: 'provider_error',
        providerId: this.id,
        message: 'Mistral API returned a malformed JSON payload.',
        model,
  ...
  private async errorFromResponse(
    ...
    return new AIProviderError({
      ...
      message:
        detail === undefined
          ? `Mistral API request failed with HTTP ${String(response.status)}.`
          : `Mistral API request failed with HTTP ${String(response.status)}: ${detail}`,
      model,
      retryable,
      httpStatus: response.status,
  ...
  /**
   * Normalise la réponse Mistral dans le contrat CORMERY.
   *
  ...
  private normalizeGeneration(
    ...
    if (message === undefined) {
      throw new AIProviderError({
        code: 'provider_error',
        providerId: this.id,
        message: 'Mistral API response did not contain a usable choice.',
        model,
        retryable: false,
      });
    }
    ...
    for (const entry of toolCalls) {
      ...
      if (id === undefined || name === undefined) {
        throw new AIProviderError({
          code: 'provider_error',
          providerId: this.id,
          message: 'Mistral API returned an incomplete tool call.',
          model,
          retryable: false,
        });
      }
  ...
  private parseToolArguments(rawArguments: unknown, model: AIModelId): JsonObject {
    ...
    if (decoded === undefined) {
      throw new AIProviderError({
        code: 'provider_error',
        providerId: this.id,
        message: 'Mistral API returned tool call arguments that are not a JSON object.',
        model,
        retryable: false,
      });
    }
  ...
  /**
   * Décode le contenu comme sortie structurée lorsqu'un format JSON a été demandé.
   *
   ...
   * métier du schéma appartient aux couches supérieures.
   *
  ...
  private extractStructured(
    ...
    if (format === undefined || format.type === 'text' || content === '') return undefined;
  ...
  private async probeStatus(signal?: AbortSignal): Promise<AIProviderStatus> {
    ...
    if (this.apiKey === undefined) {
      return {
        ...
        state: 'unavailable',
        checkedAt,
        reason: 'Mistral API credential is not configured.',
      };
    }
    ...
    try {
      ...
      if (response.status === 429) {
        return {
          ...
          checkedAt,
          latencyMs,
          reason: 'Rate limited by the Mistral API.',
        };
      }
      return {
        providerId: this.id,
      ...
      const reason =
        error instanceof AIProviderError && error.code === 'timeout'
          ...
          : 'Health check could not reach the Mistral API.';
